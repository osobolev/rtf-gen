...
public class HtmlWriter extends DocWriter {
    ...
    public boolean isOtherFont(Font font) {
        if (currentfont.isEmpty()) {
            return !standardfont.same(font);
        } else {
            Font cFont = currentfont.peek();
            return !cFont.same(font);
        }
    }
    ...
    protected void write(Element element, int indent) throws IOException {
        ...
        case Element.LIST: {
            ...
            os.write(GT);
            // contents
            for (Element elem : list.getItems()) {
                write(elem, indent + 1);
            }
            // end tag
            addTabs(indent);
        ...
        case Element.CELL: {
            ...
            String halignment = HtmlEncoder.getAlignment(cell.getHorizontalAlignment());
            if (!"".equals(halignment)) {
                write(HtmlTags.HORIZONTALALIGN, halignment);
            }
            String valignment = HtmlEncoder.getAlignment(cell.getVerticalAlignment());
            if (!"".equals(valignment)) {
                write(HtmlTags.VERTICALALIGN, valignment);
            }
        ...
        case Element.IMAGE: {
            ...
            write(HtmlTags.ALIGN, HtmlEncoder.getAlignment(image.getAlignment()));
            if (image.getAlt() != null) {
                write(HtmlTags.ALT, image.getAlt());
            }
            write(HtmlTags.PLAINWIDTH, String.valueOf(image.getPlainWidth()));
            write(HtmlTags.PLAINHEIGHT, String.valueOf(image.getPlainHeight()));
            writeMarkupAttributes(markup);
            writeEnd();
            return;
        }
    ...
    protected void writeSection(Section section, int indent) throws IOException {
        if (section.getTitle() != null) {
            ...
            // contents
            for (Element elem : section.getTitle()) {
                write(elem, indent + 1);
            }
            // end tag
            addTabs(indent);
    ...
    protected void write(Font font, Properties styleAttributes) throws IOException {
        ...
        if (styleAttributes != null) {
            for (String key : styleAttributes.stringPropertyNames()) {
